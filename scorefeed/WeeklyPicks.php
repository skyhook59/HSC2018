...
$query = "SELECT p.username, p.pick, l.favScore,l.dogscore, l.gameStatus, p.winner, l.favTeam, l.`line`, l.dogTeam FROM `picksview` p JOIN `linesview` l ON p.gameID = l.gameID WHERE p.week=$week ORDER BY p.username";
$result = mysql_query($query);
echo '<h2> Week '.$week.' Picks <small>Click names to collapse/expand</small></h2>';
...
while ($row = mysql_fetch_array($result))
{
	...
	if ($i == 0)
		...
		if( $detect->isIOS() )
		    {
				echo "<tr class=' ".$row['username']."'><td>";
				//echo $row['username'];
				//echo "</td><td>";
			}else {
				echo "<tr class='hiddenRow accordion-body collapse ".$row['username']."'><td>";
			}
